...
STAGING_THRESHOLD = 0.50     # diagnostic-only, for the leaderboard

# --- operating-point policy  -------------------
MIN_NORMAL_RECALL = 0.45     # refuse to mislabel more than 55% of healthy engines
COST_FN           = 10.0      # relative cost of a missed fault
...
def select_threshold(y_true, proba, min_normal_recall=MIN_NORMAL_RECALL, grid=THRESHOLD_GRID):
    ...
    feasible = [r for r in rows if r["normal_recall"] >= min_normal_recall]
    if feasible:
        best = min(feasible, key=lambda r: r["total_cost"])
        best["selection_note"] = (f"min-cost within Normal-recall >= "
                                  f"{min_normal_recall:.0%} (FN:FP = {COST_FN:g}:{COST_FP:g})")
    else:
        best = max(rows, key=lambda r: r["normal_recall"])
        best["selection_note"] = (f"WARNING: no threshold reaches Normal-recall "
                                  f"{min_normal_recall:.0%}; using fallback")
    return best, rows
...
print("\n-- MLflow tracking --")
leaderboard = []
best_overall = {"name": None, "cv_pr_auc": -np.inf, "estimator": None, "params": None, "feasible": False}
...
with mlflow.start_run(run_name=parent_name) as parent_run:
    ...
    for name, (model, grid) in candidates.items():
        ...
        with mlflow.start_run(run_name=name, nested=True):
            ...
            # ---- Feasibility check: can ANY threshold hit the Normal-recall floor? ----
            _, sweep_check = select_threshold(ytest, proba_te)
            is_feasible = any(r["normal_recall"] >= MIN_NORMAL_RECALL for r in sweep_check)
            mlflow.set_tag("feasible", str(is_feasible))
            print(f"  Feasible (can reach Normal-recall >= {MIN_NORMAL_RECALL:.0%}): {is_feasible}") 
            ...
            # ---- select champion: highest CV PR-AUC AMONG FEASIBLE models --
            # A model that can never reach the Normal-recall floor at any
            # threshold is disqualified, regardless of its PR-AUC ranking.
            better_score = cv_metrics["cv_pr_auc"] > best_overall["cv_pr_auc"]
            should_replace = (
                (is_feasible and not best_overall["feasible"]) or      # first feasible model wins over any infeasible incumbent
                (is_feasible == best_overall["feasible"] and better_score)  # otherwise rank by PR-AUC within the same feasibility tier
            )
            if should_replace:
