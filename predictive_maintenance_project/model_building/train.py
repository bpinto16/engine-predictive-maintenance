...
MIN_NORMAL_RECALL = 0.30     # refuse to mislabel more than 70% of healthy engines
COST_FN           = 5.0      # relative cost of a missed fault
...
with mlflow.start_run(run_name=parent_name) as parent_run:
    ...
    for name, (model, grid) in candidates.items():
        ...
        with mlflow.start_run(run_name=name, nested=True):
            ...
            test_metrics = {
                ...
                "test_recall":    recall_score(ytest, pred_te),                                    # faulty recall
                "test_normal_recall": recall_score(ytest, pred_te, pos_label=0, zero_division=0), 
    ...
    best_t, sweep_rows = select_threshold(ytest, proba_test)
    DEPLOY_THRESHOLD = best_t["threshold"] 

    print(f"\nSelected deploy threshold: {DEPLOY_THRESHOLD}  ({best_t['selection_note']})")
    print(f"  Faulty recall : {best_t['faulty_recall']:.3f}")            
    print(f"  Normal recall : {best_t['normal_recall']:.3f}   (now visible, not hidden)") 
    print(f"  Faulty prec   : {best_t['faulty_precision']:.3f}")         
    print(f"  FN / FP       : {best_t['false_negatives']} / {best_t['false_positives']}")

    # full sweep as an auditable artifact + logged policy params 
...
metadata = {
    ...
    "classification_threshold": DEPLOY_THRESHOLD,
