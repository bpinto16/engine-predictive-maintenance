...
FALLBACK_RANGES = {
    ...
    "Pressure Delta":            {"physical_min": -9.9, "physical_max": 9.9, "operating_min": 0.0, "operating_max": 5.0},
    "Temperature Ratio":         {"physical_min": 0.33, "physical_max": 3.25, "operating_min": 0.8, "operating_max": 1.2},
    "Thermal-Mechanical Stress": {"physical_min": 0.0, "physical_max": 390000.0, "operating_min": 30000, "operating_max": 130000},
}
 
# Sensible default (median-ish) starting positions + units for each slider
DEFAULTS = {
    # (default, unit, operating_min, slider_step)
    "Engine rpm":       (746.0, "RPM",   382.0, 1.0),
    "Lub oil pressure": (3.16,  "bar",   0.86,  0.05),
    "Fuel pressure":    (6.20,  "bar",   1.41,  0.05),
    "Coolant pressure": (2.17,  "bar",   0.72,  0.05),
    "lub oil temp":     (76.82, "deg C", 73.41, 0.05),
    "Coolant temp":     (78.35, "deg C", 65.74, 0.05),
    "Pressure Delta":            (0.99, "bar",    0.0, 0.05),
    "Temperature Ratio":         (0.98, "ratio",  0.0, 0.01),
    "Thermal-Mechanical Stress": (57000, "RPM-°C", 0.0, 100.0),
}
...
DISPLAY_LABELS = {
    ...
    "Pressure Delta":   "Lub/Coolant Pressure Delta",
    "Temperature Ratio": "Lub/Coolant Temp Ratio",
...
def load_metadata():
    """Load model_metadata.json shipped alongside the model.
    Falls back to a safe default threshold if the file is missing so the app
    still runs, but surfaces a visible warning to the operator."""
    try:
        path = hf_hub_download(repo_id=HF_MODEL_REPO, filename=METADATA_FILE)
        with open(path) as f:
            return json.load(f), None
    except Exception as e:
        return {"classification_threshold": 0.50,
                "feature_order": FEATURES,
                "best_model": "unknown"}, str(e)
...
FEATURES  = list(metadata.get("feature_order", FEATURES))
...
if hasattr(model, "feature_names_in_"):
    ...
    if expected != FEATURES:
        st.error(
            "**Model / metadata feature-order mismatch — refusing to run.**\n\n"
            f"Model expects: `{expected}`\n\n"
            f"Metadata says: `{FEATURES}`"
        )
        st.stop()
 
if metadata_error:
    st.warning(
        f"Could not load model metadata — using fallback threshold {THRESHOLD:.2f}. "
        f"Details: `{metadata_error}`"
    )
...
with st.sidebar:
    ...
    for feat in FEATURES:
        b = ranges.get(feat, {})
        # retraining doesn't crash the sidebar with KeyError.
        unit  = DEFAULTS.get(feat, (0, "", 0, 1.0))[1]
        label = DISPLAY_LABELS.get(feat, feat)
        if "operating_min" in b:
            st.markdown(f"**{label}**  \n{b['operating_min']:.2f} – {b['operating_max']:.2f} {unit}")
    st.divider()
   
    st.markdown("**Deployment policy**")
    st.markdown(f"Model: `{metadata.get('best_model','unknown')}`")
    st.markdown(f"Alert threshold: **{THRESHOLD:.2f}**")
    st.markdown(f"Trained on: {metadata.get('trained_on','unknown')}")
...
def sensor_slider(feat):
    b = ranges[feat]
    default, unit, _, slider_step = DEFAULTS[feat] 
    lo, hi = float(b["physical_min"]), float(b["physical_max"])

    slider_key = f"sld_{feat}"
    number_key = f"num_{feat}"
    ...
    st.markdown(f"**{DISPLAY_LABELS[feat]}** ({unit})")
    c1, c2 = st.columns([3, 1])
    with c1:
        st.slider(
            "slider", lo, hi,
            key=slider_key, step=float(slider_step), format="%.2f",   # per-feature step
    ...
    with c2:
        st.number_input(
            "number", lo, hi,
            key=number_key, step=1.00, format="%.2f",  
...
if st.button("Run diagnostics", type="primary", use_container_width=True):
    ...
    reading["Pressure Delta"] = reading["Lub oil pressure"] - reading["Coolant pressure"]
    reading["Temperature Ratio"] = reading["lub oil temp"] / (reading["Coolant temp"] + 1e-6)
    reading["Thermal-Mechanical Stress"] = reading["Engine rpm"] * reading["lub oil temp"]
    ...
    with r2:
        # Above threshold means FAULTY (bad), but delta_color="inverse" was
        # painting positive deltas green. delta_color="off" keeps the number
        # neutral so the sign doesn't imply "good/bad".
        st.metric(
            label="P(faulty)",
