...
# Constants
# ---------------------------------------------------------------------
HF_MODEL_REPO  = "bpinto16/engine-predictive-maintenance-model"
...
if not model_loaded:
    st.error(
        ...
        "Check that the model repo and filename are correct and that `preprocessing.py` "
        "(the custom QuantileCapper) is present in this Space so the pipeline can be unpickled."
    )
    st.stop()
